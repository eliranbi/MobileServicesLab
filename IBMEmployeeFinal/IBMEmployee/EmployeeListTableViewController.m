...
-(void) getEmployeeFromBlueMix{
    IMFResourceRequest* request = [IMFResourceRequest requestWithPath:@"http://mfserviceemployee.mybluemix.net/employees"];
    ...
    [request sendWithCompletionHandler:^(IMFResponse *response, NSError *error) {
        if (error != nil) {
            ...
            self.employees = (NSArray *)[NSJSONSerialization
                                         JSONObjectWithData:[[response responseText] dataUsingEncoding:NSUTF8StringEncoding]
                                         options:0 error:NULL];
            NSLog(@">>> get remote data: [employees] -  @%", self.employees);
            [self.tableView reloadData];
        }
    }];
}
